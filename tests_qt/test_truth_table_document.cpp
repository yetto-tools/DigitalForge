// Ejercita editor::TruthTableDocument: reset()/setVariableCount() con
// varias columnas de salida, alta/baja/renombre de salidas, y los limites
// de validacion de indice/valor. No necesita su propio main(): comparte el
...
TEST_CASE("A fresh TruthTableDocument starts with 4 variables and a single output F1, all cells Zero",
          "[truthTable][document]") {
...
TEST_CASE("reset() validates variableCount and resets to a single output", "[truthTable][document]") {
    TruthTableDocument doc;
    doc.addOutput("G");
...
TEST_CASE("addOutput appends a Zero-filled column and suggests F2, F3...", "[truthTable][document]") {
    TruthTableDocument doc; // 4 vars, F1
    int changedCount = 0;
    QObject::connect(&doc, &TruthTableDocument::outputsChanged, [&] { ++changedCount; });
    ...
    doc.addOutput("Segmento a");
    CHECK(doc.outputName(2) == "Segmento a");
}

TEST_CASE("removeOutput refuses to drop the last remaining output", "[truthTable][document]") {
    TruthTableDocument doc; // solo F1
...
TEST_CASE("setCellValue is independent per output and emits cellChanged with the right output index",
          "[truthTable][document]") {
    TruthTableDocument doc;
    doc.addOutput(); // F1, F2
    std::vector<std::pair<int, int>> changed;
    QObject::connect(&doc, &TruthTableDocument::cellChanged,
...
TEST_CASE("setCellValue/cellValue validate output and minterm ranges", "[truthTable][document]") {
    TruthTableDocument doc; // 4 variables, 16 celdas, 1 salida
...
TEST_CASE("setVariableCount resizes every output column, preserving surviving cells", "[truthTable][document]") {
    TruthTableDocument doc; // 4 vars
    doc.addOutput();        // F1, F2
    ...
    doc.setVariableCount(4); // grow: celdas nuevas arrancan en Zero para AMBAS salidas
    CHECK(doc.outputCells(0).size() == 16);
    CHECK(doc.outputCells(1).size() == 16);
    CHECK(doc.cellValue(0, 3) == KarnaughCellValue::One);
    CHECK(doc.cellValue(1, 3) == KarnaughCellValue::DontCare);
...
TEST_CASE("setOutputName validates the index and emits outputsChanged", "[truthTable][document]") {
    TruthTableDocument doc;
    int changedCount = 0;
    QObject::connect(&doc, &TruthTableDocument::outputsChanged, [&] { ++changedCount; });
    doc.setOutputName(0, "Segmento g");
    CHECK(doc.outputName(0) == "Segmento g");
    CHECK(changedCount == 1);
